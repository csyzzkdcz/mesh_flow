...
// This is a wrapper from https://github.com/alecjacobson/nested_cages/blob/main/flow.h, only change the signed distance fields to distance fields
...
// For a set of points P compute the direction of decrease according to the
...
// Return:
//	 ## by 1 list of distance
Eigen::VectorXd distance_direction(
...
// Return:
//   distance energy
// We remove the mass from the initial codes to make the energy and gradient be consistent
//
double grad_energy(
  const Eigen::MatrixXd & V, 
  const Eigen::MatrixXi & F, 
