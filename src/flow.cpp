...
// This is a wrapper from https://github.com/alecjacobson/nested_cages/blob/main/flow.h
	...
	Eigen::MatrixXd* grad)
{
	...
	if (quad_order == 1)
	{
		...
		Eigen::VectorXd dis = distance_direction(quad_points, V_coarse, F_coarse, grad ? &grad_Q : NULL);
		...
		Eigen::VectorXd disSq = dis;
		for (int i = 0; i < disSq.size(); i++)
			disSq[i] *= disSq[i];
		disSq = A_qv * disSq;
		energy = 0.5 * disSq.sum();
